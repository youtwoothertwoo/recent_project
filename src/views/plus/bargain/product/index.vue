      ...
      <el-form ref="form" :model="form" label-width="100px">
        <div class="table-wrap">
          <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="bargainSku[0].bargain_price" label="砍到底价" width="150">
            </el-table-column>
            ...
            <el-table-column prop="sort" label="显示状态" width="70">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.state" :active-value="10" :inactive-value="20"
                  @change="stateSet($event,scope.row.bargain_product_id)">
                </el-switch>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      getTableList() {
        ...
        if (self.$route.query.id) {
          self.bargain_activity_id = self.$route.query.id
        }
        Params.bargain_activity_id = self.bargain_activity_id;
        self.loading = true;
        BargainApi.postBargainaduit(Params, true)
          .then(data => {
            self.loading = false;
      ...
      /*编辑活动*/
      editClick(row) {
        let bargain_product_id = row.bargain_product_id;
        let bargain_activity_id = this.$route.query.id;
        this.$router.push({
          path: '/plus/bargain/product/edit',
          query: {
        ...
        self
          ...
          .then(() => {
            BargainApi.delProduct({
                  bargain_product_id: row.bargain_product_id
                },
                true
              )
              .then(data => {
