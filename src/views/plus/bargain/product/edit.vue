...
export default {
  ...
  created() {
    this.form.bargain_product_id = this.$route.query.bargain_product_id;
  ...
  methods: {
    ...
    getData() {
      let self = this;
      BargainApi.getBargainedit(
        {
          bargain_product_id: self.form.bargain_product_id
        },
        true
      ).then(res => {
    ...
    onSubmit() {
      ...
      self.$refs.form.validate(valid => {
        if (valid) {
          ...
          params.bargain_product_id=self.form.bargain_product_id;
          params.product_id=self.form.product_id;
          params.product.spec_list = self.tableFormet(self.form.tableData);
          self.loading = true;
          BargainApi.postBargainedit(params, true)
            .then(data => {
              self.loading = false;
              self.$message({
                message: '审核成功',
