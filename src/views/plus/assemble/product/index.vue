      ...
      <el-form ref="form" :model="form" label-width="100px">
        <div class="table-wrap">
          <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="assembleSku[0].assemble_price" label="拼团价" width="150">
            </el-table-column>
            <el-table-column prop="assemble_num" label="开团人数" width="120"></el-table-column>
            ...
            <el-table-column prop="sort" label="显示状态" width="70">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.state" :active-value="10" :inactive-value="20"
                  @change="stateSet($event,scope.row.assemble_product_id)">
                </el-switch>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      getTableList() {
        ...
        if (self.$route.query.id) {
          self.assemble_activity_id = self.$route.query.id
        }
        let Params = this.formInline;
        Params.page = self.curPage;
        Params.list_rows = self.pageSize;
        Params.status = self.activeName;
        Params.assemble_activity_id = self.assemble_activity_id;
        self.loading = true;
        AssembleApi.postAssembleaduit(Params, true)
          .then(data => {
            self.loading = false;
      ...
      /*编辑活动*/
      editClick(row) {
        let assemble_product_id = row.assemble_product_id;
        let assemble_activity_id = this.$route.query.id;
        this.$router.push({
          path: '/plus/assemble/product/edit',
          query: {
      ...
      deleteClick(row) {
        ...
        self
          ...
          .then(() => {
            AssembleApi.delProduct({
                  assemble_product_id: row.assemble_product_id
                },
                true
              )
              .then(data => {
      ...
      stateSet(e, id) {
        let self = this;
        AssembleApi.stateSet({
          assemble_product_id: id,
          state: e
        }).then(data => {
          self.$message({
            message: data.msg,
