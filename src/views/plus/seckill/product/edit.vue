...
export default {
  ...
  methods: {
    ...
    getData() {
      let self = this;
      SeckillApi.getSkilledit(
        {
          seckill_product_id: self.form.seckill_product_id
        },
        true
      ).then(res => {
    ...
    onSubmit() {
      ...
      self.$refs.form.validate(valid => {
        if (valid) {
          ...
          params.seckill_product_id=self.form.seckill_product_id;
          params.product_id=self.form.product_id;
          params.product.spec_list = self.tableFormet(self.form.tableData);
          self.loading = true;
          SeckillApi.postSkilledit(params, true)
            .then(data => {
              self.loading = false;
              self.$message({
                message: '审核成功',
