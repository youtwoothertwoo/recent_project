<template>
  ...
  <div class="pb50" v-loading="loading">
    <div class="product-content">
      ...
      <!--订单分润-->
      <div class="common-form">订单分润</div>
      <div class="table-wrap">
        <template v-for="item in agentInfo">
          <el-row v-if="item.user_id">
              <div class="block">
                  <h3>{{ item.label }}</h3>
                  <div class="info">
                    <p>用户昵称：{{ item.nickName }}</p>
                    <p>用户手机号：{{ item.mobile }}</p>
                    <p>分润金额：{{ item.first_money }}</p>
                  </div>
              </div>
          </el-row>
        </template>
      ...
      getParams() {
        ...
        OrderApi.orderdetail({
          ...
          .then(data => {
            ...
            self.agentInfo =[{
              ...data.data.detail.agent_new_order.agentFirst,
              first_money: data.data.detail.agent_new_order.first_money,
              label:'一级'
            },
            {
              ...data.data.detail.agent_new_order.agentSecond,
              first_money: data.data.detail.agent_new_order.second_money,
              label:'二级'
            },
            {
              ...data.data.detail.agent_new_order.agentThird,
              first_money: data.data.detail.agent_new_order.third_money,
              label:'三级'
            },
            {
              ...data.data.detail.agent_new_order.agentFour,
              first_money: data.data.detail.agent_new_order.four_money,
              label:'四级'
            },
            {
              ...data.data.detail.agent_new_order.agentSubsidy,
              first_money: data.data.detail.agent_new_order.subsidy_money,
               label:'补贴'
            }]
          })
          .catch(error => {
