<template>
  ...
  <div class="user">
          ...
          <el-table-column fixed="right" label="操作" width="150">
            <template slot-scope="scope">
              <div>
                ...
                <!-- <el-button @click="openClick(scope.row)" type="text" size="small">编辑</el-button> -->
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <Add v-if="open_view" :open_edit="open_view" :form="contractModel" @closeDialog="closeDialogFunc($event, 'add')"></Add>
    <Open v-if="isOpen" :open_edit="isOpen" :form="openModel" @closeDialog="closeDialogFunc($event, 'open')"></Open>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      PlusApi.contractList(Params, true)
        .then(data => {
          self.loading = false;
